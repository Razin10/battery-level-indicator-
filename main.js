    ...
    navigator.getBattery().then((batt) =>{
        updateBattery = () =>{
            ...
            if(level == 100){ /* We validate if the battery is full */
                batteryStatus.innerHTML = `ohh,তোর ব্যাটারি চার্জ ফুল!  <i class="ri-battery-2-fill green-color"></i>`
                batteryLiquid.style.height = '103%' /* To hide the ellipse */
            }
            else if(level <= 20 &! batt.charging){ /* We validate if the battery is low */
                batteryStatus.innerHTML = `ভাই,মোবাইলের ব্যাটারিটা নষ্ট করিস না!  <i class="ri-plug-line animated-red"></i>`
            }
            else if(batt.charging){ /* We validate if the battery is charging */
                batteryStatus.innerHTML = `এতক্ষণে চার্জে লাগাইলা মিয়া?  <i class="ri-flashlight-line animated-green"></i>`
            }
